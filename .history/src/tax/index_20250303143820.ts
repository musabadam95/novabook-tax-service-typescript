...
import { EventType, PatchSaleEvent, SaleEvent, TaxPayment, TransactionRequest } from '../interfaces/Tax';
...
router.patch<PatchSaleEvent, MessageResponse>('/sale', (req, res) => {
  ...
  const queryDate = updatedSaleEvent.date;
  const enochDate = Date.parse(queryDate);
  let saleEvent = saleEventList.find((sale) => sale.invoiceId === updatedSaleEvent.invoiceId);
