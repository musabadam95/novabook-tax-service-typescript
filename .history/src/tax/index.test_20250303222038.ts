...
describe('Adding events', () => {
  ...
  it('should return invalid date when sending a sales event', async () => {
    await request(app)
      .post(path + 'transactions')
      .send({
        eventType: 'SALES',
  ...
  it('should return invalid date when sending a TAX_PAYMENT event', async () => {
    await request(app)
      .post(path + 'transactions')
      .send({
        eventType: 'TAX_PAYMENT',
    ...
    const response = await request(app)
      .get(path + 'tax-position')
      .query({ date: '2023-02-2' });

    expect(response.status).toBe(200);
    expect(response.body.taxPosition).toBe(20);
  });

  it('should return invalid date when requesting tax position', async () => {
    const response = await request(app)
      .get(path + 'tax-position')
      .query({ date: '2023-02-2' });
